...
class _AdminMechanicsScreenState extends State<AdminMechanicsScreen> {
  final mechanics = const ['Петров Петр Петрович', 'Иванов Иван Иванович', 'Механики других клубов'];
  int? open;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: ListView.separated(
        ...
        itemBuilder: (_, i) {
          final isOpen = open == i;
          if (!isOpen) return _row(mechanics[i], onTap: () => setState(() => open = i));
          return _card(
            title: mechanics[i],
  ...
  Widget _card({required String title, required VoidCallback onCollapse, required VoidCallback onGoHistory}) {
    return Container(
