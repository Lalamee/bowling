...
public class MaintenanceRequestService {
    ...
    private AccountTypeName resolveAccountType(MechanicProfile mechanic) {
        ...
        @Transactional
        public MaintenanceRequestResponseDTO orderParts(Long requestId, Map<String, Object> payload) {
	...
	@Transactional
	public MaintenanceRequestResponseDTO markDelivered(Long requestId, Map<String, Object> payload) {
		MaintenanceRequest req = maintenanceRequestRepository.findById(requestId)
				.orElseThrow(() -> new IllegalArgumentException("Request not found"));
		List<Long> partIds = ((List<Object>) payload.getOrDefault("partIds", List.of()))
		...
		for (Long pid : partIds) {
			...
			part.setDeliveryDate(LocalDateTime.now());
			requestPartRepository.save(part);
		}
		req.setStatus(MaintenanceRequestStatus.IN_PROGRESS);
		maintenanceRequestRepository.save(req);
	...
	@Transactional
	public MaintenanceRequestResponseDTO markIssued(Long requestId, Map<String, Object> payload) {
		MaintenanceRequest req = maintenanceRequestRepository.findById(requestId)
				.orElseThrow(() -> new IllegalArgumentException("Request not found"));
		List<Long> partIds = ((List<Object>) payload.getOrDefault("partIds", List.of()))
				.stream().map(o -> Long.valueOf(o.toString())).toList();
        ...
        @Transactional
        public MaintenanceRequestResponseDTO closeRequest(Long requestId, Map<String, Object> payload) {
                MaintenanceRequest req = maintenanceRequestRepository.findById(requestId)
                                .orElseThrow(() -> new IllegalArgumentException("Request not found"));
